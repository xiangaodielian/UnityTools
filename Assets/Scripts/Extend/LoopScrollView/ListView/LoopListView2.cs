...
namespace LoopScrollView
{
    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public System.Action<GameObject, int> mOnDragSelfAction = null;
        public System.Action<List<RaycastResult>> mOnEndDragSelfAction = null;
        ...
        */
        ...
        /// 当前拖拽的item索引
        /// </summary>
        public int curDragItemIndex = -1;

        /// <summary>
        /// 不可拖动索引
        /// </summary>
        public int disDragIndex = -1;
        ...
        {
            //Raycast
            LoopListViewItem2 item = eventData.pointerCurrentRaycast.gameObject.GetComponent<LoopListViewItem2>();
            curDragItemIndex = -1;
            isDragSelf = false;
            if (item != null && item.ItemIndex != disDragIndex)
            {
                curDragItemIndex = item.ItemIndex;
        ...
        {
            if (dragItem != null && dragItem.activeSelf)
            {
        ...
        private void SetDraggedPosition(PointerEventData data)
        {
            ...
            var rt = dragItem.GetComponent<RectTransform>();
        ...
        public virtual void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (dragItem != null && IsDragItem(eventData))
            {
                dragItem.gameObject.SetActive(true);
                dragItem.transform.SetAsLastSibling();
                SetDraggedPosition(eventData);
                ScrollRect.enabled = false;
                mOnDragSelfAction?.Invoke(dragItem, curDragItemIndex);
                return;
            }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            if (isDragSelf)
            {
                ...
                isDragSelf = false;
                mOnEndDragSelfAction?.Invoke(results);
                return;
            }
            if (eventData.button != PointerEventData.InputButton.Left)
            {
